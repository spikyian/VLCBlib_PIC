...
static void consumerPowerUp(void) {
#ifdef VLCB_DIAG
    for (awriter=0; awriter<=NUM_CONSUMER_DIAGNOSTICS; awriter++) {
	consumerDiagnostics[awriter].asUint = 0;
    }
    consumerDiagnostics[CONSUMER_DIAG_COUNT] = NUM_CONSUMER_DIAGNOSTICS;
#endif
#ifdef CONSUMER_EVS_AS_ACTIONS
    areader = 0;
    awriter = 0;
    ...
    for (tableIndex=0; tableIndex < NUM_EVENTS; tableIndex++) {
        if (validStart(tableIndex)) {
            ...
            for (e=1; e<PARAM_NUM_EV_EVENT; e++) {
                if ((evs[e] >= action.a) && (evs[e] < action.a+number)) {
